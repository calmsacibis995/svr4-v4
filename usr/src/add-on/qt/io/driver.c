...
qt_init_drive()        /* ul90-34517 fix */
{
	...
	if (qt_check ()) 
		printf("Wangtek PC-36/EV-811 cartridge tape controller was found at address %xH.\n", qt_0_sioa);
	else {
		sel_archive();	/* No PC-36 or equivalent, try Archive SC499R*/
		if (qt_check ()) {
			msoutb(RSTDMA,01);
			printf("Archive SC499-R/VP402 cartridge tape controller was found at address %xH.\n", qt_0_sioa);
		}
		else {
			board_type = BT_UNKNOWN;
			printf("\nWARNING: Cartridge controller was not found at address %xH.\n\t Tape driver disabled.\n", qt_0_sioa);
		}
	}
	RETURN(1, board_type);
}
...
qt_start ()
{
	...
	execq();
	while (isclr(F_ERROCC) && isset(F_PROGRESS) && iqt->sleep) {
		splon();
		if (!ctl_exit ()) {
			sploff();
			break;
...
int	/* Issue command cmd to the controller port */
qt_sndcmd (cmd)
