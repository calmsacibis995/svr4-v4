...
#ident	"@(#)scsi:io/sd01/sd01.c	1.3.2.9"
...
/*
 * Update block count perf data here since sd01send() will switch the
 * bytes of cmd->sm_len if this job turns out to be the one to send now.
 */
...
struct cred *cred_p;
{
	...
	ret_val = physiock(sd01breakup, 0, dev, B_READ, dk->dk_vtoc.v_part[part].p_size, uio_p);
...
struct cred *cred_p;
{
	...
	ret_val = physiock(sd01breakup, 0, dev, B_WRITE, dk->dk_vtoc.v_part[part].p_size, uio_p);
...
dev_t dev;
{
	...
	if (dk->dk_vtoc.v_sanity != VTOC_SANE)
		sd01open1(getmajor(dev), getminor(dev), DKGEN);

	if (dk->dk_vtoc.v_sanity == VTOC_SANE &&
		dk->dk_vtoc.v_part[part].p_flag & V_VALID)
		return(dk->dk_vtoc.v_part[part].p_size);
	else
		return(-1);
}
