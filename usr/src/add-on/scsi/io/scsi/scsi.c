...
#ident	"@(#)scsi:io/scsi/scsi.c	1.4.2.4"
...
char 		*scsi_tmp_buff;
ulong		scsi_tot_mem;	/* total size of system memory.		*/
...
sdi_init ()
{
	 ...
	 */
	scsi_tot_mem = (ulong)ptob(maxclick);
	  ...
	  for (t = 0; t < MAX_TCS; t++)
	  {
		...
		for (i = 0; (i < tc_size) && (found == FALSE) ; i++, tcp++)
		{
			if (scsi_comp (&sc_edt[c][t].tc_inquiry[VID_LEN], 
				&tcp->tc_inquiry[VID_LEN], PID_LEN - IGN_LEN) == PASS)
...
struct proc	*procp;
{
	...
	for (i = 0, pp = &lp->pair[0]; (i < NUM_DMA_PAIR_PER_LIST) && count; ++i)
	{
		...
		if ((ulong)(tmpphysaddr+tmpcount) > scsi_tot_mem ) 
		...
		if ((sp->sp_read == FALSE)
		&& ((ulong)(tmpphysaddr+tmpcount) >= (scsi_tot_mem - NBPP))) {
			char *countp;
