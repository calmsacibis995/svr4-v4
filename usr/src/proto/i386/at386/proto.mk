...
#ident	"@(#)proto:i386/at386/proto.mk	1.3.7.8"
...
SBIN = fdisk.dy flop_disk disksetup.dy uadmin autopush sh sync umount mknod mount wsinit partsize memsize init

AT386PROGS = $(ROOT)/etc/inittab3 $(INSTALLSCRIPTS)

INSTALLSCRIPTS = $(ROOT)/tmp/hd.instl.2 $(ROOT)/tmp/hd.instl.3 $(ROOT)/tmp/Plist.dev $(ROOT)/etc/conf/cf.d/mdevice.base

ALL = $(TEXTS) $(BOOTS) LABEL TIMEZONE ask_drive $(USRBIN) $(USRSBIN) $(FSS5) $(FSUFS) $(FSBFS) $(FSCK_SP) $(SBIN) dys chan.ap $(USRLIB) $(TEXTFILES) $(DFLT_BOOTS) att at386 compaq 
...
LABEL: FLOPPY/.floppydate
	( BASEFLOPS=`ls FLOPPY/cpiolist?? | wc -l` ; \
	  ...
	  echo "$(VER).4 386unix Fnd Set 1 of $${BASEFLOPS}\n$${LOC}" > LABEL \
	)
...
FLOPPY/.floppydate: ../Plists/Plist drive_info
	...
	- (A=`pwd`; ST=`expr \`ls -r FLOPPY/cpiolis* | sed -n 1p | cut -c16-\` + 1`; BLKS=`cut -f3 drive_info` ; \
		cd $(ROOT); \
		List="usr/src/pkg/scsi.in.src usr/src/pkg/scsi.src ";\
		List1="var/spool/pkg/*/install var/spool/pkg/*/pkg* ";\
		List2="tmp/PKG_INFO";\
		if [ -d usr/src/pkg/scsi.in.src ] ;\
		then find $$List $$List1 $$List2 -print | \
...
$(ROOT)/tmp/hd.instl.2: hd.instl.2
	install -f $(ROOT)/tmp -m 555 -u bin -g bin hd.instl.2

hd.instl.2: desktop/hd.instl.2
	cp desktop/hd.instl.2 hd.instl.2

$(ROOT)/tmp/hd.instl.3: hd.instl.3
	install -f $(ROOT)/tmp -m 555 -u bin -g bin hd.instl.3

hd.instl.3: desktop/hd.instl.3
	cp desktop/hd.instl.3 hd.instl.3
...
clobber: clean newissue newdev
	...
	rm -f DISTLIST unix INSTALL INSTALL2 make_flops $(ROOT)/tmp/Plist.dev Plist.dev initial ../initial
	rm -f ask_drive $(ROOT)/var/sadm/install/contents
