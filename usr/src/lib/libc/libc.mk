...
archive_lib:
	...
	$(PROF)find port $(MACHINE) -name '*.p' -print | xargs sh -sc 'cp "$$@" object'
	#
...
shared_lib:
	...
	find port $(MACHINE) -name '*.o' -print | \
	xargs sh -sc 'cp "$$@" object'
	find port $(MACHINE) -name '*.P' -print | \
	xargs sh -sc 'cp "$$@" object'
...
abi_lib:
	...
	find port $(MACHINE) -name '*.o' -print | \
	xargs sh -sc 'cp "$$@" object'
	find port $(MACHINE) -name '*.A' -print | \
	xargs sh -sc 'cp "$$@" object'
	cp $(RTLD_DIR)/$(MACHINE)/*.o object
	#
	# delete temporary libraries
...
clean:
	...
	-rm -rf obj*
	cd port ;  $(MAKE) clean
	if u3b15 || u3b5 || u3b2 ;   then cd m32 ;   $(MAKE) clean ; \
	elif i386 ;  then cd i386 ;  $(MAKE) clean ; fi
...
clobber:
	...
	if u3b15 || u3b5 || u3b2 ;   then cd m32 ;   $(MAKE) clobber ; \
	elif i386 ;  then cd i386 ;  $(MAKE) clobber ; fi
