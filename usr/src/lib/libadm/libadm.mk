...
#ident	"@(#)libadm:libadm.mk	1.3.4.1"
.c.a:;
...
clobber: clean
	- if expr "`cc -V 2>&1`" : '.*CDE..5.0.*' > /dev/null; then : ; else CC=cc AR=ar /bin/make -f libadm3.2.mk clobber; fi
	rm -f $(LIBADM)

strip:

install: all 
	- if expr "`cc -V 2>&1`" : '.*CDE..5.0.*' > /dev/null; then : ; else CC=cc AR=ar /bin/make -f libadm3.2.mk install; fi
