...
#ident	"@(#)gencat:cat_build.c	1.1.1.4"
...
extern int sco_format;
extern int svr_format;
...
static void cat_malloc_build();
static void cat_mmp_build();
  ...
  char *catname;
{
  ...
  if (list)
    printf("Messages read from existing catalogue\n\n");
...
cat_mmp_build (fd,catname)
  ...
  char  *catname;
{
  struct cat_set *set_last = NULL;
  struct cat_msg *msg_last = NULL;
