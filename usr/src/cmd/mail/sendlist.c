...
int		level;
{
	...
	while ((r->next !=(struct recip *)NULL) && dflag != 2) {
		recip *sv_cur_recip = cur_recip;
		int sv_cur_surrogate = cur_surrogate;
		surg_rc = 0;
		r = r->next;
		cur_recip = r;		/* set global pointers for low level routines */
		recipname = r->name;	/* set global pointers for low level routines */
		Dout(pn, 5, "recip_list->name = '%s'\n", r->name ? r->name : "");
		if (!send(plist, letnum, r->name, level))
			ret++;
		Dout(pn, 5, "r->next = %#lx, dflag=%d\n",
			(long)r->next, dflag);
		cur_recip = sv_cur_recip;
		cur_surrogate = sv_cur_surrogate;
	}
	return ret;
}
