...
/*
    ...
    DESCRIPTION
	Send letter "letnum" from the current letter file to a given recipient
...
char		*name;
{
	...
	switch (sendsurg(plist, letnum, DELIVER, local, cur_recip->skip)) {
	case CONTINUE:	/* surrogate requests normal processing, */
			/* or no surrogate */
		break;

	case SUCCESS:	/* surrogate did it all */
		sendsurg(plist, letnum, POSTDELIVER, local, 0);
