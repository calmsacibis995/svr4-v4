...
copy:
			...
			/*
			 * If `target' (as passed in) is a directory, then `s2'
			 * will contain the directory's stat info., so we
			 * must stat the actual target.  This allows us to
			 * make the correct decision below when unlinking
			 * because of a failed read or write.
			 */
			if (ISDIR(s2))
				if (fstat(fo, &s2) < 0) {
					/*
			...
			for (;;) {
				...
				if (n == 0) {
					...
					/*
					 * If the first write fails because of
					 * EFBIG or ENOSPC, then errno will
					 * not be set until the second
					 * write (see write(2) man page).  The
					 * only purpose of the second write is
					 * to set errno properly.
					 */
					int nw;	/* Number of bytes written */

					errno = 0;
					if ((nw = write(fo, buf, n)) != n) {
						/*
						 * If write failed but errno
						 * wasn't set, then we either
						 * ran out of space on the
						 * device or bumped into the
						 * ulimit.  It is possible that
						 * we ran out of space on the
						 * first write and some space
						 * was freed before the second
						 * write, so we set errno just
						 * in case the second write
						 * succeeds.
						 */
						if (errno == 0) {
							errno = ENOSPC;
