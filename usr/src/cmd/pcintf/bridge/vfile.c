...
#ident	"@(#)pcintf:bridge/vfile.c	1.1.1.2"
#include	"sccs.h"
SCCSID(@(#)vfile.c	4.8	LCC);	/* Modified: 15:48:44 1/14/91 */
...
#if defined(XENIX) || defined(MIPS)
	    if(vfCache[i].pathName != (char *)0) /* avoid xenix and mips barf (strcmp) */
#endif  /* XENIX */
...
/* Scan entire cache updating all entries tha match the old path name */
    for (i = 0, vdslot = vfCache; i < vfCacheSize; i++, vdslot++) {
#if defined(XENIX) || defined(MIPS)
	if (vdslot->pathName != (char *)0)   /* prevent xenix and mips barf (strcmp) */
#endif  /* XENIX */
...
int hour, min, sec;
{
    ...
    utime = ((days + 3652L) * 86400L) + (hour * 3600L) + (min *60) + sec + timezone;
    ts = localtime(&utime);
...
/* prevent xenix and mips barf on strcmp call */
#if defined(MIPS) || defined(XENIX)
