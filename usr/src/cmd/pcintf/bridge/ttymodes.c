...
#ident	"@(#)pcintf:bridge/ttymodes.c	1.1.1.2"
#include	"system.h"
#include	"sccs.h"
SCCSID(@(#)ttymodes.c	4.4	LCC);	/* Modified: 18:19:53 4/12/91 */
...
#define	MAXVMIN		0x7f	/* Maximum unsigned char. */
	vmin = ((long)TTY_VTIME * (int)ttyspeeds[ttymodes->c_cflag&CBAUD])
		/ (BPC*VMINTICK);
	if (vmin < TTY_VMIN)
		vmin = TTY_VMIN;
