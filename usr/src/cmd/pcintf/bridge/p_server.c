...
#ident	"@(#)pcintf:bridge/p_server.c	1.1.1.1"
#include	"system.h"
#include	"sccs.h"
SCCSID(@(#)p_server.c	4.5	LCC);	/* Modified: 6/27/90 19:25:23 */
...
#ifndef JANUS
/* with memlink, donot need to test for or do retransmit */
/* Resend last message? */
 ...
 */
	...
	case NEW:
		...
		pci_delete(in->text, in->hdr.attr, addr, in->hdr.req);
		break;
	
	    case CHDIR:
		Vlog(("chdir: %s\n", "10: %s\n", in->text));
		pci_chdir(in->text, addr, in->hdr.req);
		break;
	
	    case MKDIR:
		Vlog(("mkdir: %s\n", "11: %s\n", in->text));
		pci_mkdir(in->text, addr, in->hdr.req);
		break;
	
	    case RMDIR:
		Vlog(("rmdir: %s\n", "12: %s\n", in->text));
		pci_rmdir(in->text, addr, in->hdr.req);
		break;
      ...
      ("search/find next: ptrn %s id %#x off %lx mode %#x attr %#x pid %#x\n",
		...
		pci_fsize(in->text, in->hdr.b_cnt, addr, in->hdr.req);
		break;
		...
		Vlog(("set_status: file %s mode 0%o\n", "20: %s %o\n",
		    in->text, in->hdr.mode));
		pci_setstatus(in->text, in->hdr.mode, addr, in->hdr.req);
		break;
...
/* commented out...  
		...
		*/
	    ...
	    case LOCK:
		/* If the highest bit of in->hdr.offset is 1, converts it to 0.
		   This makes sure that the UNIX side always get a nonnegative 
		   value. On a raw DOS, even if the highest bit is 1, locking 
		   still can be done successfully. */
		if ((long)in->hdr.offset < 0)
			in->hdr.offset &= ~(1L << (sizeof(long) * 8 - 1));
	    ...
	    case DOS_NLS_INFO:
		dos_nls_info(in->text, in->hdr.t_cnt, in->hdr.b_cnt, addr);
		break;
