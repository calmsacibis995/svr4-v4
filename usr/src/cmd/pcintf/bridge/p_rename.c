...
#ident	"@(#)pcintf:bridge/p_rename.c	1.1.1.2"
#include	"system.h"
#include	"sccs.h"
SCCSID(@(#)p_rename.c	4.4	LCC);	/* Modified: 17:31:14 4/12/91 */
	...
	struct output *addr;
{
	...
	cvt_fname_to_unix(mode, source, s_name);
	cvt_fname_to_unix(mode, target, d_name);
