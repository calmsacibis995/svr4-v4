...
#ident	"@(#)pcintf:bridge/p_termout.c	1.1.1.2"
#include	"system.h"
#include	"sccs.h"
SCCSID(@(#)p_termout.c	4.8	LCC);	/* Modified: 4/12/91 17:38:29 */
...
#if defined(MOTO)
/* On the Motorola 68K we need to sleep a couple of seconds     */
/* otherwise the following read will fail with errno set to EIO */
sleep(2);
#endif /* MOTO */
...
if (rd_flag) {
	...
	if (r_cnt > 0) {	/* if greater than zero let's read */
...
#if	defined(RIDGE) || defined(ULTRIX20)
		...
		if (errno == EINTR)
              	    continue;
		saved_errno = errno;
#endif	/* !RIDGE || !ULTRIX20 */

		rdsem_unlnk(semaphore);		/* remove semaphore */
		rd_shmdel(rd_shmid);		/* remove shared memory */

#if defined(MIPS) || defined(MOTO)
...
#endif
#if defined(MOTO)
	/* for some reason, this happens upon receiving a ctl-D     */
	/* on the Motorola. Just be quiet and go away */
	if (errno == EIO)
	    exit(0);
#endif
	    fatal(lmf_format_string((char *) NULL, 0, 
