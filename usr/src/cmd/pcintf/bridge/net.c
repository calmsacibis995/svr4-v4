...
#ident	"@(#)pcintf:bridge/net.c	1.1.1.1"
#include	"system.h"
#include	"sccs.h"
SCCSID(@(#)net.c	4.6	LCC);	/* Modified: 13:02:49 10/3/90 */
...
register int
	...
	for (retryCount = 0; retryCount < MAXXRETRY;) {
		...
		/* retryCount++;   This statement is never reached - SP */
	}
}
...
#	endif
#  endif /* ! SYS5_4 */
		if (ioctl(theNetDesc, I_STR, (struct strioctl *)&sic) < 0)
#else
		ifc = &ifc_data;
		...
		*intFaceNumP = 0;
		...
		for (; n > 0; n--, ifr++) {
			...
			log("netaddr (BCAST43): subnet mask: %s\n",
		...
		*inAddrP = ntohl(*inAddrP);
		...
		log("netaddr (!BCAST43): broadcast address: %s\n",
			nAddrFmt(&intFaceListP->broadAddr));
		log("netaddr (!BCAST43): subnet mask: %s\n",
			nAddrFmt(&intFaceListP->subnetMask));
	}
...
#ifdef	RS232_COPY_PROTECTION
	if (rs232_violation ==TRUE)
		fatal(lmf_get_message("NET5",
			"RS232 copy protection violation.\n"));
#endif
	return length;
}
