...
#ident	"@(#)pcintf:bridge/p_consvr.c	1.1.1.2"
#include	"system.h"
#include	"sccs.h"
SCCSID(@(#)p_consvr.c	4.11	LCC);	/* Modified: 15:21:00 12/3/90 */
...
#include	<fs.h>
#include 	<wait.h>
#endif	  /* defined(BERK42FILE)  && !defined(LOCUS) */
...
#ifdef	CCI
#define SUPERBLOCK	(SBLOCK * DEV_BSIZE)
#else
...
int
	fs_entries;			/* Number of entries in table */
...
register char *i;		/*temp for sin_zero init */
	...
	printf("PC-Interface Release %d.%d.%d\n", 
		VERS_MAJOR, VERS_MINOR, VERS_SUBMINOR);
	fflush(stdout);
...
#define	ARGVSIZE	21	/* 20 args to getty, plus null arg terminator */
...
#if defined(SYS5_4)	/* eat up dev from slvpty */
			if ((std_in = open(stdio+5, 2)) < 0) {
#else
...
#endif	/* !BERKELEY42 */
#if	!defined(EXL316) && !defined(AIX_RT) && !defined(MOTO)
...
bldDiskTab()
{
	...
	devicedesc;			/* File descriptor of device */
        ...
        long totalblks, freeblks, availblks, avail, usedblks;
	...
	*fstab_entry;			/* Entry of /etc/fstab */
	...
	/* Use /etc/mtab to determine the number of mounted file systems
	   and then add 1 (for the root file system) */

    	if (stat(MTAB, &filstat) < 0)
		return;

    	/* Calculate the number of entries in /etc/mtab */
    	fs_entries = filstat.st_size/sizeof(struct mtab) + 1;

    	/* Get enough memory for blocksize table */
    	diskTabSiz = fs_entries * (int) sizeof(struct dev_entry);
    	if ((disktab = (struct dev_entry *)memory(diskTabSiz)) == NULL)
		return;
    ...
    for (i = 0; i < fs_entries; i++) {
	    ...
	    if ((fstab_entry = getfsent()) == NULL) { /* no more entries */
		fs_entries = i; /* update number of entries; not all mounted */
		break;
	    }

	    if (strcmp(fstab_entry->fs_type, FSTAB_RW) &&
	        strcmp(fstab_entry->fs_type, FSTAB_RO) &&
	        strcmp(fstab_entry->fs_type, FSTAB_RQ))
		    continue;
    
	    if (stat(fstab_entry->fs_spec, &filstat) < 0)
	        continue;
    
	    if ((devicedesc = open(fstab_entry->fs_spec, O_RDONLY)) < 0)
	        continue;
    
	    if (lseek(devicedesc, SUPERBLOCK, 0) != SUPERBLOCK) {
	        close(devicedesc);
	        continue;
	    }
        ...
        /* Store device number and name of filesystem */
    	
	    disktab[i].rdev = filstat.st_rdev;
    
	    /* This is what df does;  bkg */
	    totalblks = superblock.fs_dsize;
	    freeblks = superblock.fs_cstotal.cs_nbfree * superblock.fs_frag +
	    		superblock.fs_cstotal.cs_nffree;
	    usedblks = totalblks - freeblks;
	    availblks = totalblks * (100 - superblock.fs_minfree) / 100;
	    avail = availblks > usedblks ? availblks - usedblks : 0;
	    freebytes = avail * superblock.fs_fsize;
    
	    disktab[i].disksize =  totalblks * superblock.fs_fsize;
	    disktab[i].diskfree = freebytes;
    ...
    if (diskTabDesc < 0) {
    	if ((diskTabDesc = open(DISKTAB, O_WRONLY|O_CREAT|O_TRUNC, 0666)) < 0) {
		log("Could not open DiskTab.\n");
		return;
    	}
    }
    else
    ...
    if ((written = write(diskTabDesc,disktab,diskTabSiz)) != diskTabSiz)
	log("Cound not write DiskTab.\n");
