...
#ident	"@(#)pcintf:pkg_rlock/shm_lock.c	1.1.1.1"
#include <sccs.h>

SCCSID("@(#)shm_lock.c	1.10");	/* 6/27/90 19:21:13 */
...
{	struct flock hostLock;
	 ...
	 * alternative, except possibly to set the start to LONG_MAX ...
	 */

	hostLock.l_start	= (long)lockLow;
	if ((lockLow + lockCount) > (unsigned long)LONG_MAX)
		hostLock.l_len	= 0L;
	else
