...
#ident	"@(#)pcintf:pkg_rlock/rlockshm.c	1.1.1.1"
#include <sccs.h>

SCCSID("@(#)rlockshm.c	1.12");  /* 2/14/89 18:37:43 */
...
{	int optChar, retCode, cfgIndex;
	...
	{
		printf(usageP, argV[0]);
		printf("Config is valid only with new style shared memory.\n");
		return EX_BAD;
	}
...
{	r0 int index;
	...
	printf("\n");
	printf("These data may be changed with these configurable");
	printf(" parameter names:\n");
	printf("\n");
	for (index = 0; !nil(_rlCfgNames[index].cnNameP); index++)
		printf("\t%s - %s\n",
			_rlCfgNames[index].cnNameP, _rlCfgNames[index].cnDescP);
}
