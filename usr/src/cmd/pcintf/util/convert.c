...
#ident	"@(#)pcintf:util/convert.c	1.1.1.2"
#ifdef NOSCCSID
#define SCCSID(arg)
#else
#define SCCSID(arg)	static char Sccsid[] = arg
#endif
SCCSID("@(#)convert.c	1.42	LCC");	/* Modified: 4/12/91 16:44:04 */
/*	@LCCID(convert.c, 1.42, 4/12/91, 16:44:04) */
/*
 convert description
...
#define OPTIONS	"?uUlLfFbB7xXi:I:o:O:c:C:mMaAzZdDpPqQsSvV"
#define P_OPTS	"?ulfb7xiocmazdpqsv" /* options we show */
#define MINOPTIONS "z"
#define	DELIMITERS "\t "
...
char input_tbl_name[_MAX_FNAME] = { NULL };
char output_tbl_name[_MAX_FNAME] = { NULL };
...
char *out_file;
{
	...
	if ( !(xlat & DONT) ) {
		switch (xlat & (D2U|U2D)) {
		case D2U:
			if (*input_tbl_name == NULL)
				strcpy(input_tbl_name, code_page_name());
			if (*output_tbl_name == NULL)
		 ...
		 */
		if ( *output_tbl_name == NULL && *input_tbl_name == NULL ) {
