...
#ident	"@(#)pcintf:pkg_lcs/prim.c	1.1.1.1"
/* SCCSID(@(#)prim.c	4.8	LCC)	* Modified: 17:09:08 4/12/91 */
/*
...
int in_len;
{
	...
	for (oh = lcs_output_table->th_output; oh; oh = oh->oh_next) {
		...
		if (oh->oh_flags & OH_DIRECT_CELL) {
			...
			b1 = ((unsigned)(c + oh->oh_char_bias) >> 8) & 0xff;
