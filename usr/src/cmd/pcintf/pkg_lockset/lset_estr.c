...
#ident	"@(#)pcintf:pkg_lockset/lset_estr.c	1.1.1.1"
#include <sccs.h>

SCCSID("@(#)lset_estr.c	1.4");	/* 2/7/89 20:37:40 */
...
{	char unknownStr[sizeof(UNKNOWN_STR)+MAX_CHARS_INT+1];
	...
	switch (error)
	{
		...
		case LSERR_TABLE_LOCK:	return "table is locked/unlocked";
		case LSERR_CHG_LOCK:	return "can't lock/unlock table";
		...
		case LSERR_SPACE:	return "no memory/table space";
