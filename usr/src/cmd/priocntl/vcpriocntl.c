...
#ident	"@(#)priocntl:vcpriocntl.c	1.1"
#include	<stdio.h>
#include	<unistd.h>
#include	<sys/types.h>
...
print_vcinfo()
{
	...
	if (priocntl(0, 0, PC_GETCID, &pcinfo) == -1)
		fatalerr("\tCan't get configured VC user priority range\n");
...
print_vcprocs()
{
	...
	if (priocntl(0, 0, PC_GETCID, &pcinfo) == -1)
		fatalerr("%s: Can't get VC class ID\n", basenm);
...
short		vcupri;
{
	...
	strcpy(pcinfo.pc_clname, "VC");
	if (priocntl(0, 0, PC_GETCID, &pcinfo) == -1)
		fatalerr("%s: Can't get VC class ID, priocntl system call \
failed with errno %d\n", basenm, errno);
