...
static	int	Xflag, Fflag, iflag, hflag, Iflag;
...
/* End Enhanced Application Compatibility Support */
	...
	/* Enhanced Application Compatibility Support */
	if (Cflag && !xflag) {
		(void) fprintf(stderr, "tar: compress (C option) is currently supported only for the xtraction (x).\n");
		usage();
	}
	/* End Enhanced Application Compatibility Support */
...
usage()
{
	(void) fprintf(stderr, "Usage: tar -{txruc}[0-9vfbk[FX]hLielmnopwAC] [tapefile] [blocksize] [tapesize] [argfile] [exclude-file] [-I include-file] files ...\n");
	done(1);
}
...
*/
		...
		case '0': case '\0':
		case '1':	/* No actual type information available for links */
...
passtape()
{
	...
	/* Enhanced Application Compatibility Support */
	if (Cflag) {
		if (eac_dblock.eac_dbuf.linkflag == '1')
			return;
...
noseek:
	if (nxb) {
		...
		/* Enhanced Application Compatibility Support */
		if (Cflag) {
			if (read(mt, eac_tbuf, TBLOCK*nblock) < 0) {
...
char *buffer;
{
	...
	if (recno >= nblock || first) {
		...
		/* Enhanced Application Compatibility Support */
		if (Cflag) {
			if ((i = read(mt, eac_tbuf, TBLOCK*j)) < 0) {
...
char *lname;	/* Name of link to be made. 	    */
{
	...
	if (firstime) {
		/* Create pipe */
		if(pipe(apipe) == -1) {
			e = errno; perror("pipe"); exit(e);	
		}
		switch (child_pid = fork()) { 
		case -1:
			/* Error fork failed */
			e = errno; perror("fork"); exit(e);	
		case 0:
			/* child */
...
/*
			      	error("%s: cannot create link", Clp->linkname);
*/
			      	(void) fprintf(stderr, "tar: cannot create link %s \n", Clp->linkname);
				continue;
			}
			if (vflag) printf("%s linked to %s\n", 
	...
	if (strlen(lname) != 0) {
		...
		if (Clp == NULL) 
/*
		     	error("malloc failed. Link information lost for %s",
*/
		     	(void) fprintf(stderr, "malloc failed. Link information lost for %s\n", lname);
		else {
			Clp->nextp = Clphead;
			Clphead = Clp;
