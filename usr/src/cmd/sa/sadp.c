...
#ident	"@(#)sa:sadp.c	1.14.4.1"

/*
 *	usage : sadp [-th][-d device[-drive]] s [n]
 *		sadp profiles the SCSI disk queues maintained by the SD01
 *		driver.  As of SVR4 sadp cannot provile any disks which
 *		use the system buffer headers to form their queues (e.g.
 *		the IDISK driver).  Prior to SVR4 sadp used to be able to
 *		read the kernel buffere headers and profile the queue that
 *		the IDISK driver kept there.  Now that buffere headers are
 *		dynamically allocated there is no longer a handle by which
 *		a user level program can access then AND even if we could
 *		we'd have to read all the chunks of buffere headers in
 *		differen reads of /dev/kmem giving us potentially corrupted
 *		queues.  A different approach will be needed to address this
 *		problem, most likely kernel instrumentation of some kind.
 */
 ...
 *	sadp no longer supports these due to reason
 *	cited at the beginning of this file
 */
#define SD01		0
#define NUMDEVTYPES 	1
char devnm[NUMDEVTYPES][5] = {
	"sd01"			/* SCSI disk */
};
...
fprintf (stderr, "dev = %d	dflg = %d\n", dev, dflg);
	...
	if (dflg == 0){
		...
		if (i == NUMDEVTYPES)
			extmsg("do not know how to profile the system disk(s)");

		/*
		 * if default drive is SD01 then set drive 0 to be
		 * profiled and call SD01_profiler
		 */
		if (dev == SD01) {
			Sdrvlist[0] = 1;
			SD01_profiler();  /* This exits when completed */
		}
	}
	else 
		if (setup[dev+SD01_D].n_value == 0){
...

{
	...
	if (setup[SD01_CNT].n_value != 0) {
		lseek(f,(long)setup[SD01_CNT].n_value, 0);
...

{
	...
	if (setup[L_SCSI].n_value != 0) {
		...
		if (read(f, &dp, sizeof(struct disk *))
				!= sizeof(struct disk *)) {
			extmsg("cannot read SD01 data structure pointer");
		}
		lseek(f, (long)dp, 0);
	...
	for (i=0; i < SNDRIVE; i++) {
		if ( (Sdrvlist[i] == 1) && ( i < Sd01_diskcnt) ) {
			if (dsd01[i].dk_state == 0) {
				fprintf(stderr,"%s-%d is not equipped\n",devnm[SD01], i);
				exit(4);
			}
		}
		else 
