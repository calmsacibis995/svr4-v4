...
/* Check if denaminator is 0 blank out the field */
		...
		if ( (tdiff != 0) 
			...
			float busy, avque;
			busy=(float)(nx.devio[ii][2]
				- ox.devio[ii][2])/(tdiff*hz/HZ) *100.0;
			avque= ((float)(nx.devio[ii][3] - ox.devio[ii][3])/
	...
	case 'd':
		...
		for (j=0; j<SINFO; j++) {
				...
				if (j == WNS)
					...
					float busy, avque;
					busy = (float)ax.devio[ii][2]/
						(tdiff*hz/HZ) * 100.0;
					avque= ((float)ax.devio[ii][3]/
						(float)ax.devio[ii][2]) - 1.0;
					printf(" %7.0f %7.1f %7.0f %7.0f %7.1f %7.1f\n",
					(busy > 100) ? 100 : busy,
					(avque < 0.0) ? 0.0 : avque,
