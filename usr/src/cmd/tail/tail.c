...
#define OCf	0x1	/* Optional Character keys */
#define OCr	0x2	
...
{
	register i,j,k;
	register Args=0; /* Mask of selected options */
	long	n,di;
	int	fromend;
	int	partial,bylines;
	int	bkwds, lastnl;
	...
	if ((Args & OCf) && (Args & OCr)) {
		fprintf(stderr, "ERROR: -f and -r are mutually exclusive.\n");
		goto errcom;
	}
