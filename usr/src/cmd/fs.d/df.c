...
#ifdef i386
	char *usage = "Usage:\ndf [-F FSType] [-begiklntVv] [current_options] [-o specific_options] [directory | special ...]\n";
#else
	char *usage = "Usage:\ndf [-F FSType] [-begiklntV] [current_options] [-o specific_options] [directory | special ...]\n";
#endif
...
#else
	while ((arg = getopt(argc,argv,"F:o:?beikVtgnlf")) != -1) {
#endif
		...
		if (v_flg && !i_flg)
			...
			printf("Mount Dir  Filesystem        blocks     used     free %%used iused  ifree %%iused\n");
		else if (!v_flg && i_flg)
			printf("Mount Dir  Filesystem            iused     ifree    itotal %%iused\n");
...
/* Enhanced Application Compatibility Support */
	case 'vi':
		...
		printf("%-8.8s %-17.17s %8ld %8ld %8ld %3d%% %6ld %6ld %3d%%\n",
			mountb->mnt_mountp, 
			mountb->mnt_special,
			TotalBlocks,
			UsedBlocks,
