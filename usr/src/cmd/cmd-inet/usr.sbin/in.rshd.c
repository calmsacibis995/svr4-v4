...
#ident	"@(#)cmd-inet:usr.sbin/in.rshd.c	1.8.10.1"
...
char	*envinit[] =
#ifdef SYSV
	    {homedir, shell, "PATH=:/bin:/usr/bin", logname, username, 0};
#else
	    {homedir, shell, "PATH=:/usr/ucb:/bin:/usr/bin", logname, username, 0};
#endif /* SYSV */
extern char	**environ;
...
#define MAX(a,b) (((u_int)(a) > (u_int)(b)) ? (a) : (b))
	...
	if (cp)
		...
		cp = pwd->pw_shell;
	execle(pwd->pw_shell, cp, "-c", cmdbuf, (char *)0, environ);
	perror(pwd->pw_shell);
