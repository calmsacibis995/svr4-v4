...
/* Enhanced Application Compatibility Support */
#define MAX_NAMELEN	14	/* max length of file name; needed for -T option */
static unsigned Mediasize = 0; 	/* default: ignore media size; needed for -K option */	
static unsigned Mediaused = 0; 	/* accumulated media user, Kbytes	*/
/* End Enhanced Application Compatibility Support */
	...
	while ((Buffr.b_end_p - Buffr.b_in_p) >= Bufsize) {
		...
		if ((rv = g_read(Device, Archive, Buffr.b_in_p, Bufsize)) < 0) {
			...
			if (errno == ENOSPC) {
				...
				filelen = Gen.g_filesz;
				dist = !(s = filelen / Bufsize) ? Bufsize : s * Bufsize;

				if (lseek(Archive, dist, SEEK_REL) < 0)
...
bflush()
{
	...
	while (Buffr.b_cnt >= Bufsize) {
		...
		/* Enhanced Application Compatibility Support   	*/
		if (Mediasize)
			Mediaused += (rv + 0x3ff) >> 10;  /* round upto 1K units */
		/* End Enhanced Application Compatibility Support   	*/
...
register long mask;
{
	...
	if (Mediasize)	{
		if (mask & OCi)	{
			Mediasize = 0;
			msg(ERR, "-K is irrelevant with the -i");
		}
		if (mask & OCp)	{
			Mediasize = 0;
			msg(ERR, "-K is irrelevant with the -p");
		}
		if ((mask & OCo) && (Bufsize & 0x3ff)) {
			if (mask & OCC) 
				msg(ERR, "Illegal  -C %d, must be multiple of 1K", Bufsize);
			else
				msg(ERR, "Must use -C option with 1K multiple when using -K option"); 
...
register char **largv;
{
	extern int optind;
	extern char *optarg;
	register char	*opts_p = "abcdfiklmoprstuvABC:DE:H:I:K:LM:O:R:STV6",
	...
	while ((option = getopt(largc, largv, opts_p)) != EOF) {
		...
		/* Enhanced Application Compatibility Support */
		case 'K':	/* media size option; forces Bufsize to be 1K multiple */
				/* only relevant to media writes; K is not needed when reading */
			/* Args |= OCK; */  
			/* could be done for completeness; but not needed */

			Mediasize = atoi(optarg);
			if (Mediasize == 0) 
				msg(ERR, "Illegal argument to -%c, \"%s\"", option, optarg);
			break;
		/* End Enhanced Application Compatibility Support */
