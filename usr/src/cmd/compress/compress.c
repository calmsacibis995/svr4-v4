 ...
 */
#ident	"@(#)compress:compress.c	1.4.3.2"
    ...
    *filelist = NULL;
	...
	if (**argv == '-') {	/* A flag argument */
		...
		switch (**argv) {
			...
			if (!ARGVAL()) {
			    fprintf(stderr, 
				"Compress -P: Missing pipe file descriptor\n");
			    exit(1);
			}
			rpipe = atoi(*argv); Pflag=1;
			goto nextarg;
		    default:
...
do_Pflag() {
	...
	time_t timep[2];
	char lng_ctmp[100], *dirname();				/* S003 */
    	struct stat statbuf;
	   ...
	   if (do_decomp) decompress(); /* Do actual compress/decompress. */
	   else           compress();
