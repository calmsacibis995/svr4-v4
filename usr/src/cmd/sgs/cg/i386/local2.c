...
#ident	"@(#)cg:i386/local2.c	1.44.3.1"
...
*/
	...
	** precision, where the Temp may be in memory.
	*/ 
	pl = getadr(pnode, ppc);
	zzzgaddr = (NODE *)0;
	zFGflag = 0;
	...
	emit_str("\n\tfxch\n");
	for(temp1 = fpsp; temp1 >= 1; temp1--)		/* don't move %st */
		fpstack[temp1+1] = fpstack[temp1];
	fpstack[1].fpop = pl->tn.op;			/* temp in %st(1) */
	fpstack[1].fpoff1 = pl->tn.lval;
	fpstack[1].fpoff2 = 0;
	fpstack[1].isqnode = 0;
	fpstack[1].ftype = TVOID;
	fpsp++;
	...
	if (fpsp > 0) {
		for(temp1 = 0; temp1 < fpsp; temp1++) {
			...
			if (fpstack[temp1].fpop != 0) {
				switch (fpstack[temp1].ftype) {
				case TFLOAT:
				    fprintf(outfile, "\tfstps\t%ld(%%ebp)", 
				    fpstack[temp1].fpoff1 - (maxboff/SZCHAR));
				    break;
				case TDOUBLE:
				    fprintf(outfile, "\tfstpl\t%ld(%%ebp)", 
				    fpstack[temp1].fpoff1 - (maxboff /SZCHAR));
				    break;
				case TLDOUBLE:
				    fprintf(outfile, "\tfstpt\t%ld(%%ebp)", 
				    fpstack[temp1].fpoff1 - (maxboff /SZCHAR));
				    break;
				}
				if( zflag ) {
		...
		if (fpstack[0].fpop != 0) {
			fprintf(outfile, "\tfstp%c\t%ld(%%ebp)",
			    (fpstack[0].ftype == TFLOAT) ? 's' : 
				(fpstack[0].ftype == TDOUBLE ? 'l' : 't'),
