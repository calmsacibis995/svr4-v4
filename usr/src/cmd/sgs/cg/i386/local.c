...
#ident	"@(#)cg:i386/local.c	1.32.2.3"
...
{
    int sz;
    int padding = 0;	/* number bytes padding needed */
...
int sz;
{
	...
	switch (sz) {
	case SZDOUBLE:	typename = "double";	break;
	case SZLDOUBLE:	typename = "ext";	padding = 2;	break;
	case SZFLOAT:	typename = "float";	break;
	}
#ifdef FP_XTOA 
	fprintf(outfile,"\t.%s\t%s\n", typename, FP_XTOA(d));
#else
