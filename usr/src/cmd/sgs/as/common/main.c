...
#ident	"@(#)nas:common/main.c	1.11.1.1"
/*
...
#endif
{
	...
	case 0:
		...
		*/
		...
		/*
		* Set up the argument list.  After the command name, the
		* special m4 macro file is added as the first file to process.
		*/
...
#endif
{
	...
	Uchar *ident = 0;		/* -Qy generated .ident string */
	size_t identlen;
	int preprocess = 0;
	...
	while ((i = getopt(argc, argv, impdep_optstr)) != EOF)
	{
		...
		{
		case 'Q':
			if (optarg[0] == 'y')	/* create own .ident */
			{
				identlen = strlen(sgsname)
					+ strlen(impdep_cmdname) + 2
					+ strlen(sgu_rel) + 1;
				ident = (Uchar *)alloc((void *)0, identlen);
			...
			*/
			while ((i = *optarg) != '\0')
			{
				if (i >= 'a' && i <= 'z')	/* common */
					...
					warn("debug character ignored: %c", i);
				optarg++;
			}
#else
	...
	*/
	initnums();
	initsyms();	/* must be after initnums() */
	initsect();
	...
	*/
	...
	{
		int	status=0;
		static const char MSGinhib[] = " Assembly inhibited ";

		fp = m4(m4path, m4defs, argc - i, &argv[i]);
		while ((outfile = mkoutname(argv[i])) == 0)
		{
			if (++i >= argc)
				break;
		}
