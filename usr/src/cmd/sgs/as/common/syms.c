...
#include "align.h"	/* default common symbol alignment */
		...
		*/
		...
		if ((sp->sym_defn = vp->val_sec) == 0)
		{
			...
			backerror((Ulong)sp->sym_file, sp->sym_line,
				...
				(const char *)sp->sym_name,
				(const char *)vp->val_sym->sym_name);
		}
		if (vp->val_sym != 0)
		{
			register Symbol *base = vp->val_sym;
...
#endif
{
	...
	*/
	if (sp->align.sym_expr->ex_type != ExpTy_Integral)
	{
	...
	*/
	if (vp->val_flags & (VAL_OFLOW | VAL_TRUNC))
	{
		backerror((Ulong)sp->sym_file, sp->sym_line,
			...
			(const char *)sp->sym_name, num2hex(vp->val_num));
	}
	align = vp->val_ulong;
#ifdef ALIGN_IS_POW2
	/*
...
#else
	if (align == 0)	/* valid alignments are positive */
	{
		backerror((Ulong)sp->sym_file, sp->sym_line,
			"invalid alignment value for %s: 0",
			(const char *)sp->sym_name);
		align = BSS_COMM_ALIGN;
	}
#endif
...
#endif
{
	...
	if (sp->sym_flags & SYM_BOUND)
	{
		...
		case Bind_Local:
			...
			break;
		case Bind_Global:
			warn("redeclaration of %s",sp->sym_name);
			return;
		case Bind_Weak:
			warn("redeclaration of %s",sp->sym_name);
			return;
		...
		error("name already bound as %s: %s", bstr,
			(const char *)sp->sym_name);
		return;
	}
	sp->sym_bind = bind;
...
#endif
{
	...
	if (sp->sym_kind == SymKind_Dot)	/* .set .,.+intexpr */
	{
		...
		*/
		...
		if (vp->val_sec != secp
			|| vp->val_flags & (VAL_OFLOW | VAL_TRUNC)
			|| vp->val_pic != 0 || vp->val_mask != 0
			|| vp->val_ulong < secp->sec_size)
		{
			error(MSGbadval);
			return;
		}
...
#endif
{
	...
	{
#ifdef DEBUG
		if (DEBUG('s') > 0 && buflen != 0)
		{
			(void)fprintf(stderr, "savestr(): waste %lu bytes\n",
				(Ulong)buflen);
		}
#endif
		if ((buflen = 2 * BUFSIZ) <= len)
		...
		if (DEBUG('a') > 0)
		{
			...
			(void)fprintf(stderr, "Total string space=%lu\n",
				total += buflen);
		}
#endif
