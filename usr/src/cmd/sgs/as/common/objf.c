...
#ident	"@(#)nas:common/objf.c	1.4"
/*
* common/objf.c - object file handling
*
*	for now:this file assumes FORMAT == ELF
...
static const Expr *src_file;	/* specified source file */
...
#endif
{
	...
	if (DEBUG('o') > 0)
	{
		(void)fprintf(stderr,
			...
			src_file == 0 ? "" : "+file", (Ulong)num_of.user_sect,
		...
		else
		{
			(void)fprintf(stderr, ", file=\"%s\"\n",
				prtstr(src_file->right.ex_str,
				src_file->left.ex_len));
		}
	}
#endif
	if (1 + (src_file != 0) + num_of.user_sect
	...
	*symp = empty;
	...
	{
		symp++;
		symp->st_name = addstr(src_file->right.ex_str,
			src_file->left.ex_len + 1);
		symp->st_value = 0;
		symp->st_size = 0;
	...
	*/
	...
	for (; cp->code_kind != CodeKind_Unset; cp = cp->code_next)
	{
		...
		*/
		...
		{
			fatal("symbol index for relocation out of range: %#lx",
				symndx);
		}
...
#endif
{
	...
	if (src_file != 0)
		return 0;
	fixexpr(ep);	/* save the string */
	src_file = ep;
	num_of.symbols++;
	symtab.sect_base++;
	symtab.local_base++;
	symtab.nonloc_base++;
	strtab.size += ep->left.ex_len + 1; /* include strsave()-added \0 */
	return 1;
}
...
#endif
{
	...
	*/
	...
	case SymKind_Set:
		...
		*/
		if (vp->val_minbits > TARGET_ADDR_BIT
	...
	if (sp->sym_exty == ExpTy_Integral)
		...
		fatal("objfsymbol():section number too bit: %lu", val);
	else
		symp->st_shndx = val;
#ifdef DEBUG
	if (DEBUG('o') > 1)
