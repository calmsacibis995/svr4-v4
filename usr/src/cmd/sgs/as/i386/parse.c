...
/* Enable "sym=val" form of .set. */
#define	TOKEN_ASSIGN	TOKEN_EQUAL
...
static const struct token_info toktab[TOKEN_DOLLAR + 1] =
{
	...
	/*ExpOp_LeafNumber*/	{"integer constant",	TOKTY_BUFFER},
...
#endif		/* re -> reg | exp */
{
	...
	else if (regno == Reg_st)	/* special 80?87 stack register */
	{
		...
		else
		{
			...
			if ((ep = exprparse()) == 0 || curtok != TOKEN_RPAREN)
				return 0; /* "syntax error" if no expr or ')' */

			opflags |= Amode_FPreg;
			curtok = yylex();
		}
		op = operand(ep);
		op->oper_flags = (Uchar) opflags;
		return op;
	}
	...
	if (ct == TOKEN_COLON)	/* ep is segment prefix */
	{
		...
		{
			curtok = ct;
			return 0; /* "syntax error" if bad segment prefix */
		}
		seg = ep;
		opflags |= Amode_Segment;
		curtok = ct = yylex();
		ep = 0;				/* no current expr */
		switch( ct )
		{
		case ExpOp_LeafRegister:
			...
			ep = regexpr(regno);
			curtok = ct = yylex();
			/* Don't allow another '(' after register. */
			if (ct == TOKEN_LPAREN)
				return 0;
			break;
		default:
		...
		if (chtab[*p] & CH_DIG)
		{
			if (numtoken())	
			{
				...
				error("invalid old-style @ token: @%s",
					prtstr(p, token_len));
			}else{ /* operand is integral constant */
				/* check for special @0xnnnnnnnn case */
				if(inbuf[token_beg] == '@'){
					token_beg++;
					token_len--;
				}
				return ExpOp_LeafNumber;
			}
		}
