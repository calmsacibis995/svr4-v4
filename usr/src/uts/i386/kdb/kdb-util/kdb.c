...
/* Initial empty symbol table; replaced by unixsyms */
static char empty_symtab[16];
char *symtable = empty_symtab;
int symtab_size = sizeof empty_symtab;
	...
	ulong	dbg_entry;
{
	...
	ulong	search_end;	/* limit on search for return address */

	if (db_st_startfp == 0) {
		db_st_offset = 0;
		db_get_stack();
		if (db_st_startsp >= USTACKLO && db_st_startsp < USTACKLIM)
			stacklim = USTACKLIM;
		else
			stacklim = (db_st_startsp & ~0xFFF) + 0x1000;
	} else
		stacklim = USTACKLIM;
	fp = db_st_startfp;
	sp = db_st_startsp;
	...
	rs = 0;
	while (INTEXT(pc)) {
		if (INSTACK(sp - sizeof(ulong), fp))
			prevfp = G(fp, 0);
		else
			prevfp = 0;
		if (INSTACK(fp, prevfp) ||
		    !INSTACK(sp, (ulong)&((ulong *)fp)[EFL]) ||
		    !INSTACK(sp, G(fp, ESP))) {
			...
			search_end = fp + sizeof(ulong);
			if (INSTACK(sp, search_end)) {
				pctry.a_addr = G(search_end, 0);
				...
				search_end = stacklim - sizeof(ulong);
			while (sp <= search_end) {
				pctry.a_addr = G(sp, 0);
				if (INTEXT(pctry.a_addr) &&
				    db_is_after_call(pctry, &fn_entry)) {
