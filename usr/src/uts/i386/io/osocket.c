...
#ident	"@(#)kern-io:osocket.c	1.3.1.2"
...
int		fd;
{
	...
	so->so_type = type;
	if ((type == OSOCK_RAW) && !suser(u.u_cred)) {
...
extern int tickadj;			/* "standard" clock skew,
				         * msec per tick */
extern int tickdelta;			/* current clock skew,
				         * msecs per tick */
extern long timedelta;			/* unapplied time correction, msecs */
extern long bigadj;			/* bigger skew */
extern int doresettodr;			/* reset clock flag */
