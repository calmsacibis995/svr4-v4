...
static int eventopen(), eventclose(), eventwput();
...
#ifdef DEBUG
	EVENT_DEBUG( "major %d minor %d", majnum, minnum);
#endif
...
#ifdef DEBUG
EVENT_DEBUG( "event ttyd is %x", ttyd);
#endif
...
#ifdef DEBUG
EVENT_DEBUG( "checking for tty %x", rdev);
#endif
...
#ifdef DEBUG
		EVENT_DEBUG("event_check_que: emask %o",
...
#ifdef DEBUG
EVENT_DEBUG( "checking for tty %x NOT FOUND", rdev);
#endif
...
#ifdef DEBUG
	EVENT_DEBUG( "event_set_dev rdev is %d", evchanp->eq_rdev);
#endif
...
mblk_t *mp;
{
	...
	case M_IOCTL:
			...
			case EQIO_GETQP:
#ifdef DEBUG
			EVENT_DEBUG( "GOT EQIO_GETQP ioctl evchanp %x",
					 evchanp);
#endif
			    if(!mp->b_cont) {
				event_iocnack(evchanp, mp, iocp, EINVAL, -1);
...
#ifdef DEBUG
EVENT_DEBUG( "event address is %x", evchanp->eq_xqinfo.xq_uaddr);
#endif
...
#ifdef DEBUG
	EVENT_DEBUG( "Copying into user address %x",
			cqp->cq_addr);
#endif
	...
	register struct evchan *evchanp;
{
	...
	EVENT_DEBUG( " END OF event_do_iocdata");
		...
		default:
#ifdef DEBUG
	EVENT_DEBUG( "IOCDATA of type %d", iocp->ioc_cmd);
#endif
