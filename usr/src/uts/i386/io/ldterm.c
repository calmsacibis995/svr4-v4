...
#ident	"@(#)kern-io:ldterm.c	1.3.2.5"
/*
 * +++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	...
	register mblk_t *mp;
{
	...
	case LDSMAP: {
...
#ifdef DEBUG
		   I18N_DEBUG( "LDTERM: LDSMAP pullupmsg failed:state %d \n",
		...
		switch(tp->emap_i.emap_reqstate){
			default: 
#ifdef DEBUG
			I18N_DEBUG( "LDTERM: LDSMAP: bad emap state %d\n",
					tp->emap_i.emap_reqstate);
#endif
			ldterm_iocnack(q,mp,iocp,EINVAL,-1);
			return;
			case READ_EMAP_BUFS: 
				if ( tp->emap_i.emap_nbuf == -1 ) {
				   ep = (struct emp_t *)mp->b_cont->b_rptr;
				   nbuf =  (ep->e_stab >> EMBSHIFT ) ;
...
#ifdef DEBUG
				I18N_DEBUG( "LDTERM: LDSMAP uaddr=%x nbuf=%d\n",
...
alldone:
	...
	case NMSMAP: {
		...
		if (error) {
#ifdef DEBUG
		   I18N_DEBUG( "LDTERM: str_nmsetmap error=%d\n", error);
#endif
		   ldterm_iocnack(q,mp,iocp,error,-1);
		   return;
		}
	...
	register mblk_t *mp;
{
	...
	case LDSMAP:  {
		...
		if (iocp->ioc_cmd == LDSMAP) {
		   ...
		   tp->emap_i.emap_reqstate =  READ_EMAP_BUFS;
		   tp->emap_i.emap_nbuf =  -1;
		   tp->emap_i.emap_mp =  (struct mblk_t *) NULL;
		   tp->emap_i.emap_lmp =  (struct mblk_t *) NULL;
		}
		qreply(q,mp);
		return;
	}
	...
	case LDGMAP:  {
		...
		if (error) {
#ifdef DEBUG
			I18N_DEBUG( "LDTERM:str_emgetmap error=%d \n", error);
#endif
			freemsg(datamp);
			datamp = NULL;
			ldterm_iocnack(q,mp,iocp,error,-1);
		...
		ep = (emp_t) datamp->b_rptr;
		nbuf = (ep->e_stab  >> EMBSHIFT) + 1;
#ifdef DEBUG
		I18N_DEBUG( "LDTERM:LDGMAP e_stab=%d nbuf =%d \n",
				ep->e_stab, nbuf);
#endif
		crqp = (struct copyreq *) iocp;
		crqp->cq_addr = * (caddr_t *)mp->b_cont->b_rptr;
	...
	case NMGMAP: {
		...
		if (error) {
#ifdef DEBUG
		I18N_DEBUG( "LDTERM:str_nmgetmap error=%d\n",error);
#endif
			freemsg(datamp);
			datamp = NULL;
			ldterm_iocnack(q,mp,iocp,error,-1);
