...
#ident	"@(#)kern-os:startup.c	1.4.3.3"
...
#include "sys/kdebugger.h"	/* For symtable */
...
mlsetup()
{
	...
	/*
	 * If there's a symbol table, we have to move its pte's to make
	 * room for BSS.  This is because the bootstrap only passes us
	 * physical addresses, not virtual.
	 */

	if (symtable) {
		i = btoc(symtab_size);
		pt = kspt0 + btoct(sbss - KVSBASE) + i;
		dpt = kspt0 + btoct(symtable - KVSBASE) + i;
		while (i-- > 0)
			*--dpt = *--pt;
	}
...
int nextfree;
{
	...
	i = ptnum(syssegs);
	for (pgcnt = 0 , pt = &kpd0[i] ; pgcnt < syssegsz ; pgcnt += 1024, pt++)
	{
		if (!PG_ISVALID(pt)) {
			pt->pg_pte = 
...
/*
 *	Allocate proc 0 u area page table and overflow page.
 */
