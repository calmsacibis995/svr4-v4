...
#ifdef i386
/* All controllers whose devices are reported for the '-d' option must
 * appear in this list, and the 'setup' list below, BEFORE the SINFO entry.
 */
#define	ID		0
#define SD01		1
#ifndef NO258
...
struct nlist setup[] = {
	...
	{"sd01_diskcnt"},
#ifndef NO258
	/* the postion of sysinfo (index SINFO above) is also used to
	 * declare the devnm array, so it must be repositioned, because
	 * 'i258' was addeded to the devnm array.
	 */
	{"i258stat"},
#endif
	{"sysinfo"}, 
...
#define BLKPERPG 	4
#ifndef NO258
#define NDEVS 128 	/* for i258, 2 boards X 64 devices / board */
#else
