...
#ifndef NO258
struct 	iotime i258stat[NDEVS];	/* statistics from i258 driver */
#endif
...
credfl:
	...
	for(;;) {
...
#ifndef NO258
		/* seek for i258stat.  If found, then read it in.  */
		if (setup[I258STAT].n_value != 0) {
			lseek(f, (long)setup[I258STAT].n_value, 0);
			if (read(f, i258stat, sizeof(struct iotime)*NDEVS)
					!= sizeof(struct iotime) * NDEVS) {
				perror("sadc can't read I258STAT structure");
				exit(2);
			}
		}
#endif
		...
		if (setup[I258STAT].n_value != 0) {
			for (k=0; k < NDEVS; k++) {
				d.devio[i][0] = i258stat[k].io_cnt;
				d.devio[i][1] = i258stat[k].io_bcnt;
				d.devio[i][2] = i258stat[k].io_act;
				d.devio[i][3] = i258stat[k].io_resp;
				i++;
			}
		}
#endif
