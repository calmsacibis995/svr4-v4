...
#ident	"@(#)mbus:cmd/devintf/mkdtab/mkdtab.c	1.1.8.1"
...
/* hard disk */
#ifdef MBUS
	if (stat("/dev/rdsk/0s0", &sb) == 0)
		hdisk(0);
	if (stat("/dev/rdsk/1s0", &sb) == 0)
		hdisk(1);
	if (stat("/dev/rdsk/2s0", &sb) == 0)
		hdisk(2);
	if (stat("/dev/rdsk/3s0", &sb) == 0)
		hdisk(3);
#else
	crambuf[0] = FDTB;
	ioctl(fd, CMOSREAD, crambuf);
...
erasecmd=\"/usr/sbin/format %s\" copy=true mkdtab=true \
mkfscmd=\"/sbin/mkfs -F s5 %s %d:%d 2 %d\" %s %s",
dparttab[0].alias, dparttab[0].cdevice, dparttab[0].bdevice, desc, mediatype,
display, blocks, fmtcdevice, dparttab[0].cdevice, 
dparttab[0].bdevice, blocks, inodes, cyl, mdenslist, mdensdefault);
...
type=ctape removable=true mkdtab=true capacity=120000 bufsize=15872 \
