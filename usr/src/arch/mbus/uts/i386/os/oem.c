...
#ident	"@(#)mbus:uts/i386/os/oem.c	1.1.1.2"
...
char valbuf [ROOTFS_NAMESZ+1];
		...
		if (!bps_open())
			if (!bps_get_val("UNX_rootfs_type", sizeof(valbuf), valbuf)) 
				...
				/*
				 * This depends on vfs_mountroot scanning
				 * the vfssw to find the right fs type.
				 */
				strcpy(rootfstype, "");
}
#endif /* MB2 || MB2AT */
...
softreset()
{
	...
	*(short *)io_rom = RESET_FLAG;
	...
	if (ics_reset_type == ICS_LOCAL_RESET) {
		...
		/* assume bus-wide */
		/* Find the CSM command record always in slot 0 */
		gcr_reg = ics_find_rec(0, ICS_CSM_RECORD_TYPE);
		/* Do the reset */
		ics_write(0, gcr_reg+ICS_RESET_OFFSET, ics_reset_type & 0x3);
	}
#endif
#endif
}
