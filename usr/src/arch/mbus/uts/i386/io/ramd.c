...
#ident	"@(#)mbus:uts/i386/io/ramd.c	1.2"
...
#ifdef MBUS
		/*
		 *	Only pretend to allocate RAMD swap.  This is because RAMD
		 *	swap is slower than just leaving the RAM free and generally
		 *	ties up critical resources.  Best deal is to use all of RAM
		 *	for pages and as long as swapsize < RAM, we'll never panic.
		*/
		if (dev == swapdev) {
			if (ramdalloc(unit,10*ptob(1)) != 0)
				return(ENOMEM);
		} else
#endif
