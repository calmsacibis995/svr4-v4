...
#ident	"@(#)mbus:uts/i386/io/mps.c	1.3.3.4"
...
/* following referred in splx() of ml/misc.s */
int mps_need_splx = 0;
int mps_need_ipl = 0;
static mps_initted = 0;
	...
	switch(mps_msg_getmsgtyp(mbp)) {
		...
		if(mps_msg_isreq(mbp)) {
			...
			if((mps_msg_gettransctl(mbp)&(MPS_MG_RRMSK|MPS_MG_RRTMSK)) == (MPS_MG_REQ|MPS_MG_RQSF)) {
				...
				impc_sol_que(tp,SOC,1); /* put it at the head */
			} else {
...
int status;
{
	...
	switch(tp->t_flags & MG_ENTRY) {
	    case MG_RSVP:
		if(tp->t_state == MG_INITST && (status != -1) && (status&MD_SCANCL)&&
				(tp->t_imsg ==NULL)) {
 ...
 */
void
mps_msg_dispatch(nmps_ckprio)
int	nmps_ckprio;
{
	int i,s,x;
	register mps_msgbuf_t *mbp;

	if( nmps_ckprio >= MPS_CLKPRIO) {
		return;
	}
 	s = splhi(); 	
	for(i=MPS_CLKPRIO;i>nmps_ckprio || i == MPS_NRMPRIO;i--) {
		mps_need_splx &= ~(1 << i) ;
		while((mbp = mps_prioq[i].m_qhead) != NULL) {
			...
			if( mbp->mb_intr != NULL) {
				/* 
				 * The correct thing to do is to convert the priority 
				 * level passed in mps_open_chan to the spl level and
				 * use that ... but since we know that all drivers are
				 * using 6, we can use this hack 
				 */
				x = spl6();				/* FORCE to ipl 6 always */
				(*mbp->mb_intr)(mbp);
				splx(x);
			}
			else
	...
	/* set values if we cannot drain the queue due to nmps_ckprio */
	for (i=MPS_NRMPRIO;i<=nmps_ckprio;i++) {
		if (mps_prioq[i].m_qhead != NULL) {
			mps_need_splx |= (1 << i) ;
			mps_need_ipl = i;
		}
	}
...
unsigned char tid;
{
	...
	if((mps_tinfo[tindex].t_state != MG_INITST) &&
		(mps_tinfo[tindex].t_state != MG_RS_RC)) {
		splx(s);
		return(-1);
	}
