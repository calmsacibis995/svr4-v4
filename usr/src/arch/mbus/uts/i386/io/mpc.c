...
/*	Copyright (c) 1986, 1987, 1989, 1990 Intel Corporation	*/
/*	All Rights Reserved	*/
...
#ident	"@(#)mbus:uts/i386/io/mpc.c	1.3.4.5"
...
mps_msgbuf_t *mbp;
{
	...
	if (status == -1)
		cmn_err(CE_WARN,
		"impc_send: slot %d destination port 0x%x transmission error", 
			mps_msg_getdstmid(mbp), mps_msg_getdstpid(mbp));
	splx(s);
	return (status);
}

impc_sol_que(tp,que, front)
tinfo_t *tp;
int que;
int front;
{
	...
	switch(que) {
		...
		if(impc_so_head == NULL) {
			...
			if (front == 0) {
				...
				if (impc_so_head == impc_so_tail)
					impc_so_tail = tp;

				/* queue it at head + 1 */
				ttp = impc_so_head->t_next;
				impc_so_head->t_next = tp;
				tp->t_next = ttp;
			}
		}
		break;
		...
		if(impc_si_head == NULL) {
			...
			if (front == 0) {
				...
				if (impc_si_head == impc_si_tail)
					impc_si_tail = tp;

				/* queue it at head + 1 */
				ttp = impc_si_head->t_next;
				impc_si_head->t_next = tp;
				tp->t_next = ttp;
			}
		}
		break;
