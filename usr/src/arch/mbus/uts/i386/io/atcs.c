...
#ident	"@(#)mbus:uts/i386/io/atcs.c	1.3.2.3"
...
#define DMSGQ 256	/* message output blocking */
...
extern void atcsOktoSend(struct atcs_lines *);
...
{	register struct strtty 		*tp;
	...
	if (mps_msg_iscancel(tmsg)) {
		...
		DMONITOR();
		/* just fall through, some requests will recover */
		/* XXX need to recover by resending the request ?? */
	}
		...
		*/
		...
		case A_CD: {
			...
			switch(((struct ATCSWaitForCarrierResp *)mps_msg_getudp(tmsg))->State) {
				case ATCSWFCOn:
					/*
					 * Carrier Detect: Wakeup the process.  We make sure the
					 * status is successful first.  The other possible status
					 * at this point is "input flushed".
					*/
					if (((struct ATCSWaitForCarrierResp *)mps_msg_getudp(tmsg))->Status == ATCSSuccess) {
						ln->l_state |= CARRF;
						atcsSetCarr(tp, ln);
				...
				case ATCSWFCOff:
					 ...
					 * We make sure the status is successful first.  The other 
					 * possible status at this point is "input flushed".
					*/
					if (((struct ATCSWaitForCarrierResp *)mps_msg_getudp(tmsg))->Status == ATCSSuccess) {
						ln->l_state &= ~CARRF;
						if ((tp->t_state&CARR_ON) && !(tp->t_cflag&CLOCAL))
...
got_cancel:
	ln->l_OMCnt--;
	if (ln->l_state & MSGQFULL) {
		ln->l_state &= ~MSGQFULL;
		DDEBUG(DMSGQ, (CE_CONT,"atcsintr: wakeup minor %d\n", ln->l_minor));
		(void) wakeup((caddr_t)&ln->l_state);
	}
...
/*
 * atcsOktoSend() -- check if it is ok to send a message to server
 */
void
atcsOktoSend(ln)
struct atcs_lines *ln;
{
	...
	DDEBUG(DMSGQ, (CE_CONT, "atcsOktoSend: ln=%xH\n", ln));
	while (ln->l_state & MSGQFULL) {
		(void) sleep((caddr_t)&ln->l_state, TTIPRI|PCATCH);
	}
	s = SPL();
	if (++ln->l_OMCnt == ln->l_MQSize) {
		ln->l_state |= MSGQFULL;
		DDEBUG(DMSGQ, (CE_CONT,"atcsOKtoSend: msg limit %d reached minor %d\n",
			ln->l_MQSize, ln->l_minor));
	}
	splx(s);
}
