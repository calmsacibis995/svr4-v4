...
#ident	"@(#)mbus:uts/i386/io/cci.c	1.3.2.2"
...
int disc;		/* The line discipline needed */
{
	...
	/*
	 *  We can get into the situation below if there has been an open
	 *  just ahead of this one.  We'll return EAGAIN to the user and 
	 *  they can try again once the line is all set up.
	*/
	if (cp->c_chan != (uint)-1) {
		cmn_err(CE_WARN, "ccistartsock: cci channel reallocated\n");
		return (-1);
	}
	...
	int mssgl;		/* number of bytes in mssg */
{
	...
	if (mps_AMPsend_rsvp(cp->c_chan, mbp, (struct dma_buf *)0, (struct dma_buf *)0) == -1) {
		...
		/*
		 *  We free this here since mps_AMPsend_rsvp doesn't when it fails and
		 *  returns -1.  The tid must be freed for mps_close_chan to be able
		 *  to do what it does - there must be no active transactions in the
		 *  table.
		*/
		(void) mps_free_tid(cp->c_chan, our_tid);
		return(-1);
	} else {
		if (action == A_SYNC)
