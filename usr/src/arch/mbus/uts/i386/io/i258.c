...
#ident	"@(#)mbus:uts/i386/io/i258.c	1.4.3.4"
...
register unit;
{
	...
	for(i = 0;i<i258_max_req;i++) {
		if ((UNIT(dd->reqinfo[i].r_dev) == unit) &&
			(dd->reqinfo[i].r_flags & i258RQ_BUSY))
			return(1);
	}
	/*
...
struct cred	*cred_p;
{
	...
	DEBPR(DEB_OPEN,(CE_CONT, 
		"i258open: unit=%d, , flag = 0x%x, otyp = %d\n", unit, flag, otyp));
	DEBPR(DEB_OPEN,(CE_CONT, 
	...
	if ((dr->dr_part[part].p_flag & V_RONLY) && 
		(flag & FWRITE) && 
		(otyp == OTYP_MNT)) {
       		SETERR(EROFS);
	}
...
struct cred		*cred_p;
{
	 ...
	 * Make sure no one else is opening or closing this unit.
	 */
	DEBPR(DEB_CLOSE,(CE_CONT, 
		"i258close: BEFORE SF_OPENCLOSE check, otyp %d, partition %d\n",
			otyp, PARTITION(dev)));
	while (dd->d_sflags[unit] & SF_OPENCLOSE)
	...
	if (ISWINI(dev)) { /* only reset partitions on hard disk */
		...
		/* 
		 * ignore swap partition and check if any other partition is open ? 
		 */
		for (p = 0; p < OTYPCNT; p++) {
			if ((otyp != OTYP_SWP) && (dr->otyp[p] != 0)) {
				openflg++;
				break;
			}
		}
		/* flush out the buffers for block special and mounted file systems */
		if  (((otyp == OTYP_BLK) || (otyp == OTYP_MNT)) &&
			(dr->otyp[otyp] != 0)) {
			DEBPR(DEB_CLOSE,(CE_CONT, 
				"i258close: openflg %d,  otyp %d prior to flush \n",
					openflg, otyp));
	...
	if(!openflg) {
		while (i258checkreq(dd, unit)) {
			DEBPR(DEB_CLOSE,(CE_CONT, "Waiting for flush on unit %d\n",unit));
			dd->d_sflags[unit] |= SF_CLOSEWAIT;
			(void) sleep((caddr_t)&dd->d_pdev[unit],PRIBIO);
			DEBPR(DEB_CLOSE,(CE_CONT, "Woke up flush on unit %d\n",unit));
		}
		DEBPR(DEB_CLOSE,(CE_CONT, "About to release unit %d\n",unit));
 ...
 * TITLE:	i258blkio1
 *
 * ABSTRACT:    Does blocking and deblocking for i/o requests that do not
 ...
 ****************************************************************************/
void
i258blkio1(obp, offset, count, ssize)
register buf_t	*obp;
unsigned int	offset;
unsigned int	count;
unsigned int	ssize;
{
	...
	obp->b_resid = obp->b_bcount;
 	if ((bp = (buf_t *) getrbuf(KM_NOSLEEP)) == 0)	{
 		obp->b_flags |= B_ERROR;
 		obp->b_error = EIO;
 		cmn_err(CE_WARN, "i258blkio1: cannot get buffer header");
 		return;
 	}
	if ((sect = kmem_alloc(ssize, KM_NOSLEEP)) == 0) {
 		cmn_err(CE_WARN, "i258blkio1: cannot get buffer");
 		obp->b_flags |= B_ERROR;
 		obp->b_error = EIO;
 		return;
 	}
...
unsigned int	lbps;	/* blocks/sector, must be 2, 4, 8, 16 ..*/
{
	register buf_t	*bp;
	unsigned int	i, sshift, ssize, smask;
	...
	smask = ssize - 1;
 	...
 	if ((bp = (buf_t *) getrbuf(KM_NOSLEEP)) == 0)	{
 		obp->b_flags |= B_ERROR;
 		obp->b_error = EIO;
 		cmn_err(CE_WARN, "i258blkio: cannot get buffer header");
 		biodone(obp);
 		return;
 	}
	*bp = *obp;
	...
	if (i = (bp->b_blkno & (lbps-1))) {
		...
		bp->b_blkno &= ~(lbps-1);
		i258blkio1(bp, offset, count, ssize);
		if (bp->b_flags & B_ERROR) {
			obp->b_flags |= B_ERROR;
			obp->b_error = bp->b_error;
			freerbuf(bp);
...
register struct buf *bp;    /* buffer header */
{
	 ...
	 * trying to write to a read-only partition.  If so, error.
	 */
