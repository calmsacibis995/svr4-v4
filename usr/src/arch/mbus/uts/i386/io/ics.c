...
#ident	"@(#)mbus:uts/i386/io/ics.c	1.3.1.2"
...
ics_get_cpunum ()
{
	char devbuf [CPUNUMSIZE + 1];
	int cpun = 0;

	if (!bps_open()) {
		if (!bps_get_val(UNX_CPUNUM, sizeof(devbuf), devbuf)) {
			if (!bps_get_integer (devbuf, &cpun)) {
				return (cpun);
			}
		}
	}
...
static int
ics_get_dev (bpsmajor, bpsbase)
int	*bpsmajor;
int	*bpsbase;
{
	char devbuf [DEVVAL+1];
	int cc, state;
	char temp[DEVVAL + 1];
	...
	*bpsbase = 0;
	if (bps_open())
		return (-1);
	if (bps_get_val(UNX_DEV, sizeof(devbuf), devbuf)) 
		return (-1);
	state = 0;
	while (!(bps_get_opt(devbuf, &state, "major:base", &cc,
			 sizeof (temp), temp))) {
		switch (cc) {
		default:
			return (-1);
		case 1:
			if (bps_get_integer (temp, bpsmajor)) 
				return (-1);
			break;
		case 2:
			if (bps_get_integer (temp, bpsbase)) 
				return (-1);
			break;
		}
	}
...
{	
	...
	/* get the cpunumber from bps string */
	if ((cpunum = ics_get_cpunum ()) == -1) {
		/* default algorithm for computing cpunum */
		cpunum = 0;
	...
	if (ics_get_dev (&bpsmajor, &bpsbase) == -1) {
		bpsmajor = ics_bdev[cpunum].major;
		bpsbase = ics_bdev[cpunum].base;
	} 
	rootdev = makedevice (bpsmajor, bpsbase + rootdev_minor);
	swapdev = makedevice (bpsmajor, bpsbase + swapdev_minor);
	pipedev = makedevice (bpsmajor, bpsbase + pipedev_minor);
	dumpdev = makedevice (bpsmajor, bpsbase + dumpdev_minor);
}
