...
/	actual or intended publication of such source code.
	...
	.ident	"@(#)mbus:uts/i386/ml/misc.s	1.1.1.1"
...
#ifdef MB2
	cmpl    $0x0, mps_need_splx	/ if transport needs splx hook
	je     mb2done             	/  no work for transport 
	cmpl    mps_need_ipl, %eax	/ if new level >= mps_need_ipl
	jge     mb2done            	/  no work for transport 
	push	%eax
	call	mps_msg_dispatch
	pop		%eax				/ pop off parameter
	movl	4(%esp), %eax		/ get new spl level
mb2done:	
#endif
	/ fall through to spl
...
/ This must be done as early as possible, since it wouldn't be safe to
/ use a kernel debugger to single-step or set breakpoints through here.
/
	...
	movl	%esp,%esi		/   (to and from an arbitrary addr)
	movl	%esi,%edi
...
/ Handle the bulk of startup in C, in mlsetup().
	call	mlsetup
