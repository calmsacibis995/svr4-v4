...
#ident	"@(#)hiint:cmd/disksetup/disksetup.c	1.1"
...
char *argv[];
{
	...
	if (param_override == PARAM_CHANGED) {
		...
		num=0;
		for (i=0; i < 4; i++)
			if ((((struct btblk *) absbuf)->ipart[i].systid == UNIXOS) && (((struct btblk *) absbuf)->ipart[i].bootid == ACTIVE)) {
				num=i;
				break;
			}
		unix_base = (daddr_t) ((struct btblk *) absbuf)->ipart[num].relsect;
		free(absbuf);
		init_structs();
		writevtoc();
		exit(PARAM_CHANGED);
...
#ifdef MBUS
	if (scsi_flag == TRUE) {
		perror("Verify operation not supported");
		exit(80);
	}
#endif /* MBUS */
	while (n_tries-- > 0) {
...
#ifdef MBUS
	if (scsi_flag == TRUE) {
		/* pdinfo and vtoc must currently fit within dp.dp_secsiz */
		len = dp.dp_secsiz;
...
/* check if hard disk swap device added previously, can't reset */
...
int req_flag; /* does the slice require a mkfs type */
{
	if (req_flag == TRUE) 
		for (;;) {
			printf("Which type of filesystem do you want created on this slice? (s5, s52k or ufs) ");
			gets(replybuf);
...
char *rawdev, *size, *secspercyl;
{
	if (sliceinfo[slice].mkfsflag == BFSMKFS)  
		...
		if ((sliceinfo[slice].mkfsflag == S51KMKFS) || 
			...
			if (scsi_flag == TRUE)
				sprintf(replybuf,"/etc/fs/ufs/mkfs  -C  %s  %s  %d  %d  4096 %d 12 10 60 2048 t 0 0 >/dev/null 2>&1",
				rawdev,size,(int)dp.dp_sectors,(int)dp.dp_heads,
				FRAGSIZE);
			else
				sprintf(replybuf,"/etc/fs/ufs/mkfs  -C  %s  %s  %d  %d  4096 %d 16 10 60 2048 t 0 0 >/dev/null 2>&1",
				rawdev,size,(int)dp.dp_sectors,(int)dp.dp_heads,
				FRAGSIZE);
	if (system(replybuf)) {
