...
#ident	"@(#)hiint:cmd/disksetup/diskinit.c	1.1"
...
char	*absbuf;		/* chunk of space for first disk read */
...
get_unix_partition()
{
	...
	if ((absbuf=malloc(dp.dp_secsiz)) == NULL) {
		fprintf(stderr,"get_unix_partition -- can't allocate buffer\n");
		exit(71);
	}
	absio.abs_sec = 0;
	...
	/* Wait to set unix_base until we know we have a valid ipart */
	unix_size = 
...
#ifdef MBUS
	if (scsi_flag == TRUE ) {
		perror ("Disk_sync not supported");
		exit(79);
	}
#endif /* MBUS */
