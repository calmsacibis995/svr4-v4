...
#ident	"@(#)hiint:cmd/bootutils/sgib.d/mkbtblk.c	1.1"
...
mkbtblk()
{
	...
	if ((int)nfheads > 0) {
		bt->ipart[0].bootid = ACTIVE;
		bt->ipart[0].beghead = 0;
		bt->ipart[0].begsect = 1;
		bt->ipart[0].begcyl = 0;
		bt->ipart[0].systid = UNIXOS;
		bt->ipart[0].endhead = nfheads - 1;
		bt->ipart[0].endsect = nsec | (char) (((ncyl - 1) >> 2) & 0x00c0);
		bt->ipart[0].endcyl = (char) ((ncyl - 1) & 0x00ff);
		bt->ipart[0].relsect = 1;
		/* Leave room for disk's alternates */
		bt->ipart[0].numsect = (long) (nfheads * nsec * (ncyl - nalt - 3));

		for (i=1; i<=3; i++)
			(void)memset(&bt->ipart[i], 0, IPART_SIZE);
	} else {
		(void)memset(bt->ipart, 0, IPART_SIZE);
	}
	bt->ivlab.v_fsdelta =	fsdelta + (dev_gran * bt->ipart[0].relsect);
