...
#ident	"@(#)hiint:cmd/bootutils/sgib.d/util.c	1.1"
...
char	*device;
{
	...
	switch (btype) {
		...
		case BDISK:	/* Hard Disk */
			for (i=0; i < BTBLK_SIZE / dev_gran; i++) {
				absio.abs_sec = i;
				absio.abs_buf = (char *) (&btblk + (i * dev_gran));
				if ((ioctl(dev, V_WRABS, &absio)) != 0 ) {
					ERR(stderr, "write of boot buffer failed\n");
					exit(1);
				}
			}
			for (j=0; j < REAL_2_SIZE / dev_gran; j++) {
				absio.abs_sec = i + j;
				absio.abs_buf = (char *) (real_buf_2 + (j * dev_gran));
				if ((ioctl(dev, V_WRABS, &absio)) != 0 ) {
					ERR(stderr, "write of '%s' failed\n", rboot);
					exit(1);
				}
			}
			for (k=0; k < msa_text_size / dev_gran; k++) {
				absio.abs_sec = i + j + k;
				absio.abs_buf = (char *) (msa_text_buf + (k * dev_gran));
				if ((ioctl(dev, V_WRABS, &absio)) != 0 ) {
					ERR(stderr, "write of '%s' failed\n", boot);
					exit(1);
				}
