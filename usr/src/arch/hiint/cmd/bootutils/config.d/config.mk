...
#ident	"@(#)hiint:cmd/bootutils/config.d/config.mk	1.1"
...
HIINT_STAND = stnd.cfg.hiint

DEODX = $(ROOT)/usr/src/cmd/layers/deodx
BINARIES = pci258 pci258.1m pci258dbg pci133dbg
BINSRCS = pci258src pci258.1msrc pci258dbgsrc pci133dbgsrc
...
install:  
	if [ $(BUS) = MB2 ];then					 \
		...
		install -f $(BOOTCFG) -m 744 -u root -g bin $(PCIDEBUG) ;\
		install -f $(BOOTCFG) -m 744 -u root -g bin $(PCI133) ;\
		install -f $(BOOTCFG) -m 744 -u root -g bin $(DEFCONFIG) ;\
		cp $(HIINT_STAND) $(DEFCONFIG)				;\
