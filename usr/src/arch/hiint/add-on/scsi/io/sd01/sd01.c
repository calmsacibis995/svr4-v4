...
#ifndef lint
static char sd01_copyright[] = "Copyright 1990 Intel Corporation 467879-010";
#endif /* lint */

#ident	"@(#)hiint:add-on/scsi/io/sd01/sd01.c	1.1"
...
int	sd01_started;			/* True if sd01start has been called */
...
void		sd01blkio(); 		/* Blocking/Deblocking i/o	*/
void		sd01blkio1();	 	/* Blo/deblo within a sector	*/
...
/* Initialize only when called through sd01start */
	if(!sd01_started)
		return;
...
/* Fill in the scb for this job */
	...
	{				/* Logical address */
		...
		cmd->sm_addr = dk->dk_vtoc.v_part[part].p_start + (bp->b_blkno >> BLKSEC);
	}
	else
	{
		cmd->sm_addr = (bp->b_blkno >> BLKSEC);
	}
	cmd->sm_res2 = 0;
	...
	drv_getparm(LBOLT, &my_lbolt);
	bp->b_start = HZ2MSEC(my_lbolt);
...
register buf_t *bp;
{
	...
	/*
	*  If starting and/or ending point of the i/o is not
	*  on disk sector boundaries, call blocking/deblocking
	*/
	if (bp->b_blkno & BLKSMSK || bp->b_bcount % BLKSZ)
	{
		sd01blkio(bp, BLKSEC);
		return;
	}
	...
	*/
	...
	if (bp->b_flags & B_READ)    
	{                          
		if ((unsigned)(bp->b_blkno >> BLKSEC) + numblk >  last)
		{
			if ((unsigned) (bp->b_blkno >> BLKSEC) > last)
			{
				bp->b_flags |= B_ERROR;
				bp->b_error = ENXIO;
				iodone(bp);
	...
	else				/* This is a write request */
	{
		...
		if (dk->dk_vtoc.v_part[part].p_start <= dk->unixst + VTBLKNO  &&
			(bp->b_blkno >> BLKSEC) <= dk->unixst + VTBLKNO && 
...

/* %BEGIN HEADER
 * %NAME: sd01blkio - Non-sector-size io for odd counts and offsets.
 * %DESCRIPTION: 
...
unsigned int	sshift;	/* shift, to xlate blocks to sectors	*/
{
	...
	smask = ssize - 1;
 	...
 	if ((bp = (buf_t *) getrbuf(KM_NOSLEEP)) == 0)	{
 		obp->b_flags |= B_ERROR;
 		obp->b_error = EIO;
 		cmn_err(CE_WARN, "sd01blkio: cannot get buffer header");
 		biodone(obp);
 		return;
 	}
	*bp = *obp;
	...
	if (i = (bp->b_blkno & (lbps-1))) {
		...
		bp->b_blkno &= ~(lbps-1);
		sd01blkio1(bp, offset, count, ssize);
...

/* %BEGIN HEADER
 * %NAME: sd01blkio1 - Non-sector-size io within one disk sector.
 * %DESCRIPTION: 
	Does blocking and deblocking for i/o requests that do not
 ...
 * %END HEADER */
void
sd01blkio1(obp, offset, count, ssize)
register buf_t	*obp;
unsigned int	offset;
unsigned int	count;
unsigned int	ssize;
{
	...
	obp->b_resid = obp->b_bcount;
 	if ((bp = (buf_t *) getrbuf(KM_NOSLEEP)) == 0)	{
 		obp->b_flags |= B_ERROR;
 		obp->b_error = EIO;
 		cmn_err(CE_WARN, "sd01blkio1: cannot get buffer header");
 		return;
 	}
	if ((sect = kmem_alloc(ssize, KM_NOSLEEP)) == 0) {
 		cmn_err(CE_WARN, "sd01blkio1: cannot get buffer");
 		obp->b_flags |= B_ERROR;
 		obp->b_error = EIO;
 		return;
 	}
 ...
 * %DESCRIPTION: 
	...
	which get passed to sd01strategy()..
 * %CALLED BY: physiock()
 * %SIDE EFFECTS: None.
 * %RETURN VALUES: None.
 * %END HEADER */
...
struct cred *cred_p;
{
	...
	ret_val = physiock(sd01breakup, 0, dev, B_READ, dk->dk_vtoc.v_part[part].p_size << BLKSEC, uio_p);
...
struct cred *cred_p;
{
	...
	ret_val = physiock(sd01breakup, 0, dev, B_WRITE, dk->dk_vtoc.v_part[part].p_size << BLKSEC, uio_p);
...
struct cred *cred_p;	/* Pointer to user credential structure */
{
	...
	if ((*dev_p == rootdev) && (swapdev != NODEV) && DKINDEX(rootdev) == DKINDEX(swapdev)) {
		nswap = dk->dk_vtoc.v_part[DKSLICE(swapdev)].p_size << BLKSEC;
	}
...
/* Update performance stat */
	...
	drv_getparm(LBOLT, &my_lbolt);
	lbolt_time = (time_t) HZ2MSEC(my_lbolt);
...
/* If this is a read or write update the I/O queue */
		...
		(char) *jp->j_cont->SCB.sc_cmdpt == SM_WRITE)
	{
		...
		dk->dk_stat.pttrack->b_blkno = (sdi_swap32(jp->j_cmd.cm.sm_addr) << BLKSEC);
...
/* If the request is for the PD sector override the supplied sector parameter */
	...
	while(phyarg->datasz > 0)
	{
		...
		bp->b_blkno = phyarg->sectst << BLKSEC;
		bp->b_flags &= ~B_DONE;
...
int part;
{
	...
	if (part < SD01_PBLK)			/* Logical address  */
		start = (bp->b_blkno >> BLKSEC) + dk->dk_vtoc.v_part[slice].p_start;
	else					/* Physical address */
		start = (bp->b_blkno >> BLKSEC);
...
void (*strategy)();
{
	...
	*bp = *obp;
	...
	while(count > 0)
	{
		...
		bp->b_un.b_addr += bp->b_bcount;
		bp->b_blkno += (bp->b_bcount >> BLKSHF) << BLKSEC;
		count -= bp->b_bcount;
	}
 ...
 * %DESCRIPTION:
	This function calls sd01init() to do the driver initialization.
 * %CALLED BY: Kernel
 * %SIDE EFFECTS: None
 * %RETURN VALUES: None
 * %END HEADER */
