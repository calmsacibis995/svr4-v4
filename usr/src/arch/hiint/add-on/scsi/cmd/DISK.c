...
#ifndef lint
static char DISK_copyright[] = "Copyright 1990 Intel Corporation 467880-010";
#endif /* lint */

#ident	"@(#)hiint:add-on/scsi/cmd/DISK.c	1.1"
...
unsigned	bsize_req;		/* Block Size Requested	*/
char		secbuf[BLKSIZE];
...
int		fixbsize = FALSE;		/* TRUE if block size is specified */
...
char	**argv;
{
	...
	{
		strcpy(STRING, "Sintr:vVuf:");	/* FORMAT */
		do_format = TRUE;
	...
	while ((c = getopt(argc, argv, STRING)) != EOF)
		...
		case 'h' :
			break;
#endif /* ix86 */
		case 'f' :	/* Fix the block size to given value */
			fixbsize = TRUE;
			bsize_req = atoi(optarg);
			break;
...

{
	...
	int		bdl;
	int		bsize;
	char		*p1, *p2; /* used for byte pointers */
	SENSE_PLBD_T	*plbd = (SENSE_PLBD_T *) NULL;
	...
	while (!done) {
		...
		case MDSENSE :
			...
			if (mdsense_bufsz > 0) {
				...
				/* Set Block Descriptor length and pointer, Page pointer */
				bdl = ((SENSE_PLH_T *) mdsense_bufpt)->plh_bdl;
				if (bdl)
					plbd = (SENSE_PLBD_T *) (mdsense_bufpt + SENSE_PLH_SZ);
				dadf = (DADF_T *) (mdsense_bufpt + SENSE_PLH_SZ + bdl);

				if (fixbsize) {
#if defined (i386) || defined (i486)
					bsize = bdl ? scl_swap24(plbd->plbd_bsize) :
							scl_swap16(dadf->pg_bytes_s);
#else
					bsize = bdl ? plbd->plbd_bsize : dadf->pg_bytes_s;
#endif /* ix86 */

					if (bsize != bsize_req) {
						printf("\tThe current disk block size is %d\n", bsize);
						printf("\tTrying to set the block size to %d\n", bsize_req);

						/* Set requested values in the existing SENSE */
#if defined (i386) || defined (i486)
						if (bdl)
							plbd->plbd_bsize = scl_swap24(bsize_req);
						dadf->pg_bytes_s = scl_swap16(bsize_req);
#else
						if (bdl)
							plbd->plbd_bsize = bsize_req;
						dadf->pg_bytes_s = bsize_req;
#endif /* ix86 */
						dadf->pg_sec_t = 0; /* Let the target compute sec/trk */
						dadf->pg_res1 = 0; /* Clear the Page-Saveable bit */

						/* Request changeable values with a new SENSE */
						mdsense2_bufpt = malloc(mdsense_bufsz);
						mdsense2_cdb = mdsense_cdb;
						mdsense2_cdb.ss_addr |= 0x40; /* Page Control bit */
						mdsense(mdsense2_cdb, mdsense2_bufpt, mdsense_bufsz);

						/* Allocate memory for MODE SELECT Data */
						mdselect_bufsz = mdsense_bufsz;
						mdselect_bufpt = malloc(mdselect_bufsz);

						/* Copy the SENSE data to the MODE SELECT buffer */
						p1 = (char *) mdsense_bufpt;
						p2 = (char *) mdselect_bufpt;
						for (i = mdsense_bufsz; i; i--)
							*p2++ = *p1++;

						/* Clear all but changeable values in SELECT buffer */
						p1 = (char *) mdsense2_bufpt + SENSE_PLH_SZ + SENSE_PLBD_SZ;
						p2 = (char *) mdselect_bufpt + SENSE_PLH_SZ + SENSE_PLBD_SZ;
						for (i = mdsense_bufsz - SENSE_PLH_SZ - SENSE_PLBD_SZ; i; i--)
							*p2++ &= *p1++;

						/* Send the MODE SELECT CDB */
						mdselect_cdb = mdsense_cdb;
						mdselect_cdb.ss_op = SS_MSELECT;
						mdselect_cdb.ss_addr1 |= 1; /* Set Save-Parms bit */
						mdselect_cdb.ss_addr = 0; /* Clear reserved field */
						mdselect(mdselect_cdb, mdselect_bufpt, mdsense_bufsz);

						/******
						printf("Trying to set the logical block size\n");
						mdselect_cdb.ss_len = SENSE_PLH_SZ + SENSE_PLBD_SZ;
						mdselect(mdselect_cdb, mdselect_bufpt, SENSE_PLH_SZ + SENSE_PLBD_SZ);
						******/	
