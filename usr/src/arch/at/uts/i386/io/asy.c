...
mblk_t *bp;
{
	...
	case AIOCDOSMODE:
	{
		...
		{       
			...
			asy_v86_prp_pd[2*asychan(tp->t_dev)] = p_v86->v86_u_procp;
...
unsigned int	vec;
{
	...
	for (asy = &asytab[0], mdev =0;  mdev < num_asy ; asy++, mdev++) 
	...
	tp = &asy_tty[mdev];
	if ((interrupt_id == RxRDY) || (interrupt_id == FFTMOUT) || (line_status == RCA) || (interrupt_id == RSTATUS)) {
		...
		if (ringcnt != RINGSIZ) {
			*ringput++ = c;
			...
			asydchars++; /* just dropeed a character */
		}
		if (!asyiflag[mdev]) {
			asyiflag[mdev]=timeout(asyrproc, mdev, HZ/60);
		}
	...
	if (interrupt_id == TxRDY || (line_status & XHRE) && (tp->t_state & BUSY)){
		/*
