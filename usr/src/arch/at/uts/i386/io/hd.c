...
#ident	"@(#)at:uts/i386/io/hd.c	1.3.4.1"
...
char *bth_trkmap[NUMDRV];/* Bits are set for all tracks recorded in the "bad
			  ...
			  */
unsigned prev_mapsize[NUMDRV];	 /* mapsize to free if reallocating bbh maps */
...
hdi->hd_alts_len, aptr->alt_sec.alt_reserved, aptr->alt_sec.alt_base);
		...
		if (bbh_trkmap[unit] != NULL) {
			kmem_free(bbh_trkmap[unit],prev_mapsize[unit]);
			bbh_trkmap[unit] = NULL;
		}
		if (bth_trkmap[unit] != NULL) {
			kmem_free(bth_trkmap[unit],prev_mapsize[unit]);
		...
		prev_mapsize[unit] = mapsize; /* saving size of bbh maps */
...
dev_t dev;
{
	...
	hdi = &hddrvinfo[unit];
        if (!(hdi->hd_state & HD_OPEN)) {
                 ...
                 * partition table.)
                 */
                if (hdopen(&dev, 0, OTYP_LYR, NULL) == 0)
                	(void) hdclose(dev, 0, OTYP_LYR, NULL);
        }
	hdp = hdpartinfo[unit];
